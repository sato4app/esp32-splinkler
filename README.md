...
| 項目 | 内容 |
|---|---|
| 間隔（interval） | 日・分・秒で指定（日 0〜365、分 0〜1439、秒 0〜59）。1秒以上 |
| 動作時間 | 分・秒で指定。1秒以上で、回数が2回以上なら間隔より短くする |
| 回数（repeat） | 動作させる回数（1〜100） |
| 開始日時 | 1回目の動作を始める日時。手で変更もできる |
| セット（set） | 現在時刻＋間隔を開始日時に入れる |
| スタート（start） | 開始日時から間隔ごとに、回数分の動作日時を算出して一覧に出す |
| クリア | 動作予定を消す |

n回目の動作は「開始日時 + (n−1) × 間隔」に始まり、動作時間だけ続く。
開始日時が空欄のままスタートした場合は、セットと同じく現在時刻＋間隔を使う。
開始日時が過去の場合はエラーにする。

一覧では各回を「予定・次回・動作中・完了」で表示し、次回までの残り時間（動作中は終了までの残り時間）を1秒ごとに更新する。
入力値と動作予定はブラウザ（localStorage）に保存し、ページを開き直しても続きから表示する。
