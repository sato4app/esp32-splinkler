...
| 回数 | 動作させる回数（1〜100） |
| 開始日時 | 1回目の動作を始める日時。初期値は現在日時＋10秒で、手で変更もできる |
| リセット | 開始日時を初期値（現在日時＋10秒）に戻す |
| スタート | 開始日時から間隔ごとに、回数分の動作日時を算出して一覧に出す |
| ストップ | 進行中の動作予定をすぐに止める。一覧は残し、終わっていない回は「中止」にする |

間隔・動作時間・回数の初期値はすべて0で、このままスタートするとエラーになる（値を入れてから使う）。

n回目の動作は「開始日時 + (n−1) × 間隔」に始まり、動作時間だけ続く。
開始日時が空欄のままスタートした場合は、リセットと同じく現在日時＋10秒を使う。
開始日時が過去の場合はエラーにする。

一覧では各回を「予定・次回・動作中・完了・中止」で表示し、次回までの残り時間（動作中は終了までの残り時間）を1秒ごとに更新する。

動作中は開始から1秒ごとに電子音（1000Hz・0.08秒）を鳴らし、開始と終了の瞬間だけ高く長い音（2000Hz・0.5秒）にする。
ブラウザの自動再生制限のため、ページを開き直した後は画面に一度触れるまで音は鳴らない。
画面を消したり別のアプリに切り替えたりすると、ブラウザが処理を止めるため音も止まる。
